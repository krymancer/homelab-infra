...
same-pod `guacd` sidecar (`guacamole/guacd:1.6.0`). `GUACD_TUNNEL_HOST=127.0.0.1`
matches the sidecar layout (upstream compose uses the `termix` service name;
`resolveJumpTunnelEndpoint` already advertises loopback when `GUACD_HOST` is
loopback). guacd listens on loopback 4822 only and is not exposed by Service or
Ingress. It runs as UID/GID 1000 with `drop: [ALL]` like the other containers
(upstream compose does not add capabilities; the official image already uses USER
`guacd` UID 1000). The Termix data PVC is also mounted into guacd at
`/termix-data`, with `GUACD_RECORDING_PATH=/termix-data/session_recordings/guacamole`
and `GUACD_DRIVE_PATH=/termix-data/rdp-drive`, so RDP drive redirection and
Guacamole recordings land on the same volume Termix reads from `/app/data`.
Collaboration rooms use the in-memory fallback; `REDIS_URL` is omitted because
this Deployment is `replicas: 1`. TLS terminates at Traefik; `ENABLE_SSL=false`
avoids internal ACME/self-signed-certificate setup. Trusted proxy auth is off. SSH/web basics require adding a host
